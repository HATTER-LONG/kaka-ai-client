...
import {
  ensureSelectedAwangModel,
  loadStoredAccount,
  loadSelectedAwangModel,
  saveSelectedAwangModel,
  syncManagedProviders,
  switchManagedProviders,
...
const STORAGE_KEY = "kaka-ai-client-last-app";
...
const VIEW_STORAGE_KEY = "kaka-ai-client-last-view";
const SIDEBAR_STORAGE_KEY = "awang-ai-sidebar-collapsed";
const MODEL_SIDEBAR_STORAGE_KEY = "awang-ai-model-sidebar-collapsed";
...
function App() {
  ...
  const [managedSwitchingChoice, setManagedSwitchingChoice] = useState<
    "awang" | "official" | null
  >(null);
  ...
  const [isModelSidebarCollapsed, setIsModelSidebarCollapsed] = useState(() => {
    return localStorage.getItem(MODEL_SIDEBAR_STORAGE_KEY) === "true";
  });
  const [selectedAwangModel, setSelectedAwangModel] = useState(
    loadSelectedAwangModel,
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      MODEL_SIDEBAR_STORAGE_KEY,
      String(isModelSidebarCollapsed),
    );
  }, [isModelSidebarCollapsed]);
  ...
  const visibleApps: VisibleApps = AWANG_VISIBLE_APPS;
  ...
  const awangModelError =
    awangAccount && awangModels.length === 0
      ? "卡卡AI后台没有返回可用模型"
      : "";
  const awangModelDisplayError = awangAccount?.modelsError || awangModelError;

  useEffect(() => {
    if (!awangAccount || awangModels.length === 0) return;
    const saved = loadSelectedAwangModel();
    const next = awangModels.some((model) => model.id === saved)
      ? saved
      : awangModels[0].id;
    saveSelectedAwangModel(next);
    setSelectedAwangModel(next);
  }, [awangAccount, awangModels]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (view === "providers" || view === "dashboard") return;
  ...
  const invalidateManagedState = async () => {
    await queryClient.invalidateQueries({ queryKey: ["providers"] });
  ...
  const handleManagedSwitch = async (choice: "awang" | "official") => {
    setManagedSwitchingChoice(choice);
    try {
      const stored = loadStoredAccount();
      if (stored?.payload) {
        await syncManagedProviders(
          stored.payload,
          selectedAwangModel || ensureSelectedAwangModel(stored.payload),
        );
      }
      await switchManagedProviders(choice);
      await invalidateManagedState();
      toast.success(choice === "awang" ? "已一键应用卡卡AI" : "已切换官方渠道");
    } catch (error) {
      toast.error(extractErrorMessage(error) || "切换失败");
    } finally {
      setManagedSwitchingChoice(null);
    }
  };

  const handleAwangModelSelect = async (modelId: string) => {
    const stored = loadStoredAccount();
    if (!stored?.payload) {
    ...
    saveSelectedAwangModel(modelId);
    setSelectedAwangModel(modelId);
    try {
      await syncManagedProviders(stored.payload, modelId);
      await invalidateManagedState();
      toast.success(`已切换模型：${modelId}`);
    } catch (error) {
      toast.error(extractErrorMessage(error) || "同步模型配置失败");
    }
  };
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card px-4 py-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          只允许官方渠道和卡卡AI渠道，不支持添加自定义渠道商。
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <Button
                          onClick={() => void handleManagedSwitch("awang")}
                          disabled={managedSwitchingChoice !== null}
                        >
                          {managedSwitchingChoice === "awang" ? (
                        ...
                        <Button
                          variant="outline"
                          onClick={() => void handleManagedSwitch("official")}
                          disabled={managedSwitchingChoice !== null}
                        >
                          {managedSwitchingChoice === "official" ? (
                            <Loader2 className="h-4 w-4 animate-spin" />
                          ) : (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="flex flex-1 min-w-0 items-center justify-end gap-1.5">
              ...
              <div
                ref={toolbarRef}
                className="flex flex-1 min-w-0 overflow-hidden items-center py-4 pr-2"
              >
                <div
                  className="flex min-w-0 flex-1 items-center justify-end gap-1.5 ml-auto"
                  style={{ WebkitAppRegion: "no-drag" } as any}
                >
                  {currentView === "prompts" && (
                    <Button
                  ...
                  {currentView === "providers" && (
                    <>
                      ...
                      <div className="flex shrink-0 items-center gap-1 p-1 bg-muted rounded-xl">
                        <AnimatePresence mode="wait">
                          <motion.div
                            key={
                              activeApp === "openclaw"
        ...
        <div className="flex flex-1 min-h-0">
          <aside
            className={cn(
              "shrink-0 border-r border-border bg-card/50 px-3 py-4 transition-[width] duration-200",
          ...
          >
            ...
            <nav className="space-y-1">
              ...
              >
                ...
                {!isSidebarCollapsed && <span>{"\u5151\u6362\u7801"}</span>}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {!isSidebarCollapsed && <span>{"\u914d\u7f6e"}</span>}
              </button>
            </nav>
          </aside>

          <main className="flex-1 min-w-0 min-h-0 flex flex-col overflow-y-auto animate-fade-in">
          ...
          <aside
            className={cn(
              "shrink-0 border-l border-border bg-card/50 px-3 py-4 transition-[width] duration-200",
              isModelSidebarCollapsed ? "w-16" : "w-72",
            )}
          >
            <div className="mb-3 flex items-center justify-between gap-2">
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => setIsModelSidebarCollapsed((value) => !value)}
                title={isModelSidebarCollapsed ? "展开模型" : "收起模型"}
                className="h-8 w-8"
              >
                {isModelSidebarCollapsed ? (
            ...
            ) : awangModelDisplayError ? (
              <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
                {awangModelDisplayError}
              </div>
            ) : awangModels.length === 0 ? (
              <div className="rounded-lg border border-border bg-background/60 p-3 text-xs text-muted-foreground">
                登录后从后台获取模型列表。
              </div>
            ) : (
              <div className="space-y-2 overflow-y-auto pr-1">
                {awangModels.map((model) => (
                  <button
                    key={model.id}
                    type="button"
                    onClick={() => void handleAwangModelSelect(model.id)}
                    className={cn(
                      "w-full rounded-lg border px-3 py-2 text-left transition-colors",
                      selectedAwangModel === model.id
                  ...
                  >
                    ...
                    <div className="mt-0.5 truncate font-mono text-xs opacity-80">
                      {model.id}
                    </div>
                  </button>
                ))}
