...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: values.remoteRoot ?? "kaka-ai-client-sync",
...
export function WebdavSyncSection({
  ...
  const [form, setForm] = useState(() => ({
    ...
    remoteRoot: config?.remoteRoot ?? "kaka-ai-client-sync",
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = config.remoteRoot ?? "kaka-ai-client-sync";
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: form.remoteRoot.trim() || "kaka-ai-client-sync",
  ...
  const currentRemotePath = `/${form.remoteRoot.trim() || "kaka-ai-client-sync"}/v2/db-v6/${form.profile.trim() || "default"}`;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Input
              ...
              placeholder="kaka-ai-client-sync"
              className="text-xs flex-1"
              disabled={isLoading}
            />
          </div>
