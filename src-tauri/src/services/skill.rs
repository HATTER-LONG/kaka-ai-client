...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.kaka-ai-client/skills/`
...
pub enum SkillStorageLocation {
    /// 卡卡AI客户端 管理目录 (~/.kaka-ai-client/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.kaka-ai-client/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.kaka-ai-client/skill-backups/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 卡卡AI客户端 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "kaka-ai-client".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "kaka-ai-client".to_string()));
