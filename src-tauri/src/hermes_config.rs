...
///
/// Input (from 卡卡AI客户端 UI / database):
/// ```json
...
/// `providers:` dict. 卡卡AI客户端 treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by 卡卡AI客户端)
...
/// keeping 卡卡AI客户端 free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay 卡卡AI客户端 must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a 卡卡AI客户端 edit to an unrelated field would silently
...
// 卡卡AI客户端 fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later 卡卡AI客户端 edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a 卡卡AI客户端 toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
