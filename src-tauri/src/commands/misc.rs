...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/LiShiBin/kaka-ai-client/releases/latest",
            None::<String>,
        )
...
async fn fetch_github_latest_version(client: &reqwest::Client, repo: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .header("User-Agent", "kaka-ai-client")
...
echo "[kaka-ai-client] Starting: {cmd}"
echo ""
{cmd}
echo ""
echo "[kaka-ai-client] Command exited. Press any key to close."
read -n 1 -s
"#,
...
mod tests {
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("kaka-ai-client-missing-{unique}"));
