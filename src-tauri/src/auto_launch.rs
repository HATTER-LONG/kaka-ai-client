...
/// 将 `/path/to/卡卡AI客户端.app/Contents/MacOS/卡卡AI客户端` 转换为 `/path/to/卡卡AI客户端.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/卡卡AI客户端.app/Contents/MacOS/卡卡AI客户端");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/卡卡AI客户端.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/卡卡AI客户端.app/Contents/MacOS/卡卡AI客户端");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/卡卡AI客户端.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/kaka-ai-client");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/kaka-ai-client");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
